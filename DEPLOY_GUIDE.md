# YuwenChat 超级无脑部署指南

> **看这里！** 这个教程把你当成完全不懂的小白，每一步都有详细说明，复制粘贴就能完成！

---

## 🎯 部署前必读（2分钟）

### 这个项目和普通网站的区别

| 普通 PHP 网站 | YuwenChat（本项目） |
|--------------|-------------------|
| 文件扔进去就能用 | 需要安装依赖 + 构建 |
| 不需要额外进程 | 需要 PM2 跑后端 |
| 直接访问文件 | 需要配置反向代理 |

### 整体流程图

```
┌─────────────────────────────────────────────────────────────────┐
│                         部署流程（共9步）                         │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ① 安装宝塔   →   ② 安装PM2和Nginx   →   ③ 上传代码            │
│                                                                 │
│       ↓                                                         │
│                                                                 │
│  ④ 终端执行一键命令（安装依赖+构建+启动）                         │
│                                                                 │
│       ↓                                                         │
│                                                                 │
│  ⑤ 添加网站   →   ⑥ 配置Nginx   →   ⑦ 域名解析                 │
│                                                                 │
│       ↓                                                         │
│                                                                 │
│  ⑧ 申请SSL证书   →   ⑨ 开放防火墙端口   →   完成！              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

# 第一部分：准备工作

## 步骤 1️⃣ 买服务器

### 去哪买？

| 服务商 | 网址 | 特点 |
|-------|------|-----|
| 阿里云 | aliyun.com | 国内最大，需备案 |
| 腾讯云 | cloud.tencent.com | 便宜，需备案 |
| Vultr | vultr.com | 国外，免备案，可能被墙 |

### 买什么配置？

```
最低配置：1核 1G内存 1M带宽（约50元/月）
推荐配置：2核 2G内存 3M带宽（约100元/月）

⚠️ 重要：系统一定要选 Ubuntu 22.04！不要选 CentOS！
```

### 买完后记下这些信息

```
📝 记在记事本里：

服务器IP: _______________（例如：47.98.123.456）
root密码: _______________（买的时候设置的）
```

---

## 步骤 2️⃣ 买域名

### 去哪买？

和服务器同一家买最方便（阿里云/腾讯云都有）

### 买什么域名？

```
例如：chat.shawntv.co
后缀 .com 约 55元/年
后缀 .co 约 20元/年
```

### 买完后记下

```
📝 你的域名: chat.shawntv.co
```

---

## 步骤 3️⃣ 安装宝塔面板

### 3.1 连接到服务器

**Windows 用户：**
```
1. 下载 PuTTY：https://www.putty.org
2. 打开 PuTTY
3. Host Name 填你的服务器IP
4. 点 Open
5. 弹窗点 Yes
6. login as: 输入 root 回车
7. Password: 输入你的密码（不显示是正常的）回车
8. 看到 root@xxx:~# 就成功了！
```

**Mac 用户：**
```
1. 打开「终端」
2. 输入：ssh root@你的服务器IP
3. 输入 yes 回车
4. 输入密码回车
5. 看到 root@xxx:~# 就成功了！
```

### 3.2 安装宝塔

复制下面这一整条命令，粘贴到终端，回车执行：

```bash
wget -O install.sh https://download.bt.cn/install/install-ubuntu_6.0.sh && sudo bash install.sh ed8484bec
```

等待 2-5 分钟，出现下面的信息就安装好了：

```
==================================================================
Congratulations! Installed successfully!
==================================================================
外网面板地址: http://47.98.123.456:8888/abcdefgh
username: abcdefgh
password: 12345678
==================================================================

⚠️⚠️⚠️ 重要！把这些信息截图或复制保存！⚠️⚠️⚠️
```

### 3.3 记下宝塔信息

```
📝 宝塔面板地址: ___________________________
📝 宝塔用户名: _______________
📝 宝塔密码: _______________
```

---

# 第二部分：宝塔面板操作

## 步骤 4️⃣ 登录宝塔 + 安装软件

### 4.1 登录宝塔面板

```
1. 打开浏览器
2. 地址栏输入你的宝塔面板地址
3. 输入用户名和密码
4. 登录成功！

⚠️ 首次登录弹出的「推荐安装套件」窗口，点 × 关掉！
```

### 4.2 安装 PM2 管理器

```
📍 位置：左侧菜单 →「软件商店」

操作步骤：
1. 点击左侧「软件商店」
2. 搜索框输入「PM2」
3. 找到「PM2管理器」
4. 点击右边的「安装」
5. 弹窗点「确定」
6. 等待 2-3 分钟
7. 看到绿色「运行中」就成功了
```

### 4.3 安装 Nginx

```
📍 位置：左侧菜单 →「软件商店」

操作步骤：
1. 搜索框输入「nginx」
2. 找到「Nginx」（不是 OpenResty）
3. 点击「安装」
4. 版本选 1.24（或最新）
5. 勾选「极速安装」
6. 点「确定」
7. 等待 1-2 分钟完成
```

---

## 步骤 5️⃣ 上传项目代码

### 5.1 创建目录

```
📍 位置：左侧菜单 →「文件」

操作步骤：
1. 点击左侧「文件」
2. 地址栏输入：/www/wwwroot
3. 按回车进入
4. 点击上方「新建目录」
5. 名称输入：chat.shawntv.co
6. 点「确定」
```

### 5.2 上传代码

**方法A：上传压缩包（推荐小白）**
```
1. 把项目打包成 .zip 文件
2. 双击进入 chat.shawntv.co 目录
3. 点击上方「上传」
4. 选择你的 zip 文件
5. 等待上传完成
6. 右键点击 zip 文件 →「解压」→「解压到当前目录」
```

**方法B：Git 拉取**
```
1. 点击左侧「终端」
2. 执行命令：

cd /www/wwwroot/chat.shawntv.co
git clone https://github.com/yuwenchat/2026-Rev.git .

注意最后有个点！
```

### 5.3 确认上传成功

```
📍 位置：左侧菜单 →「文件」→ /www/wwwroot/chat.shawntv.co/

你应该看到这些文件夹：
✅ client/      （前端代码）
✅ server/      （后端代码）
✅ PLAN.md
✅ README.md
```

---

## 步骤 6️⃣ 执行一键部署命令 ⭐最关键的一步⭐

### 6.1 打开终端

```
📍 位置：左侧菜单 →「终端」
```

### 6.2 复制粘贴这一整段命令

```bash
# ===== 一键部署命令（全部复制，粘贴到终端，回车执行）=====

cd /www/wwwroot/chat.shawntv.co && \
echo ">>> 1/5 创建数据目录" && \
mkdir -p server/data && \
echo ">>> 2/5 安装后端依赖（约1-2分钟）" && \
cd server && npm install --registry=https://registry.npmmirror.com && \
echo ">>> 3/5 创建环境变量文件" && \
cat > .env << 'EOF'
PORT=3000
NODE_ENV=production
JWT_SECRET=yuwenchat-secret-key-change-this-in-production
CLIENT_URL=https://chat.shawntv.co
DB_PATH=./data/chat.db
EOF
echo ">>> 4/5 安装前端依赖并构建（约2-3分钟）" && \
cd ../client && npm install --registry=https://registry.npmmirror.com && npm run build && \
echo ">>> 5/5 启动后端服务" && \
cd ../server && pm2 delete yuwenchat 2>/dev/null; pm2 start src/index.js --name yuwenchat && pm2 save && \
echo "" && \
echo "========================================" && \
echo "✅ 部署完成！" && \
echo "========================================"
```

### 6.3 执行结果

如果看到这个，说明成功了：
```
========================================
✅ 部署完成！
========================================
```

如果报错，看下面的「常见错误处理」。

### 6.4 验证后端运行状态

在终端执行：
```bash
pm2 list
```

应该看到：
```
┌────┬──────────┬─────────┬─────────┬──────────┐
│ id │ name     │ status  │ cpu     │ memory   │
├────┼──────────┼─────────┼─────────┼──────────┤
│ 0  │ yuwenchat│ online  │ 0%      │ 45mb     │
└────┴──────────┴─────────┴─────────┴──────────┘

status 显示 online = 成功！
status 显示 errored = 失败，看日志：pm2 logs yuwenchat --err
```

---

## 步骤 7️⃣ 添加网站 ⭐重点看这里⭐

### 7.1 打开添加网站界面

```
📍 位置：左侧菜单 →「网站」→ 点击「添加站点」
```

### 7.2 填写信息（照着填！）

```
┌─────────────────────────────────────────────────────────────┐
│                      添加站点                                │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  域名:     chat.shawntv.co                                  │
│                                                             │
│  备注:     (可以不填)                                        │
│                                                             │
│  根目录:   /www/wwwroot/chat.shawntv.co/client/dist         │
│            ↑↑↑ 重要！一定要填到 dist！↑↑↑                    │
│                                                             │
│  FTP:      不创建                                            │
│                                                             │
│  数据库:   不创建                                            │
│                                                             │
│  PHP版本:  纯静态                                            │
│            ↑ 一定要选「纯静态」！                            │
│                                                             │
└─────────────────────────────────────────────────────────────┘

点击「提交」
```

### 7.3 常见错误

```
❌ 错误：根目录填成了 /www/wwwroot/chat.shawntv.co
✅ 正确：根目录要填 /www/wwwroot/chat.shawntv.co/client/dist

❌ 错误：PHP版本选了 PHP-74
✅ 正确：PHP版本要选「纯静态」
```

---

## 步骤 8️⃣ 配置 Nginx（反向代理）⭐超重要⭐

### 8.1 打开网站设置

```
📍 位置：左侧「网站」→ 找到你的网站 → 点击网站名称（或点「设置」）
```

### 8.2 打开配置文件

```
📍 位置：网站设置窗口 → 左侧点「配置文件」
```

### 8.3 修改配置（保留 SSL 证书配置！）

⚠️ **不要删除原有的 SSL 配置！** 只需要修改/添加以下内容：

找到这些内容并修改：
```nginx
# 1. 修改 root 路径（加上 /client/dist）
root /www/wwwroot/chat.shawntv.co/client/dist;

# 2. 修改 index（去掉 php）
index index.html index.htm;
```

然后在配置文件中找个合适的位置（比如 `#REWRITE-END` 后面），添加这三段：

```nginx
    # ========== 前端路由（Vue Router）==========
    location / {
        try_files $uri $uri/ /index.html;
    }

    # ========== 后端 API 代理 ==========
    location /api {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    }

    # ========== WebSocket 代理（聊天必须）==========
    location /socket.io {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_connect_timeout 60s;
        proxy_send_timeout 60s;
        proxy_read_timeout 60s;
    }
```

### 8.3.1 完整配置参考（已有 SSL 证书的情况）

如果你已经在宝塔申请了 SSL 证书，完整配置应该长这样：

```nginx
server
{
    listen 80;
    listen 443 ssl http2 ;
    server_name chat.shawntv.co;
    index index.html index.htm;
    root /www/wwwroot/chat.shawntv.co/client/dist;

    #CERT-APPLY-CHECK--START
    # 用于SSL证书申请时的文件验证相关配置 -- 请勿删除
    include /www/server/panel/vhost/nginx/well-known/chat.shawntv.co.conf;
    #CERT-APPLY-CHECK--END

    #SSL-START SSL相关配置，请勿删除或修改下一行带注释的404规则
    #error_page 404/404.html;
    ssl_certificate    /www/server/panel/vhost/cert/chat.shawntv.co/fullchain.pem;
    ssl_certificate_key    /www/server/panel/vhost/cert/chat.shawntv.co/privkey.pem;
    ssl_protocols TLSv1.1 TLSv1.2 TLSv1.3;
    ssl_ciphers EECDH+CHACHA20:EECDH+CHACHA20-draft:EECDH+AES128:RSA+AES128:EECDH+AES256:RSA+AES256:EECDH+3DES:RSA+3DES:!MD5;
    ssl_prefer_server_ciphers on;
    ssl_session_cache shared:SSL:10m;
    ssl_session_timeout 10m;
    add_header Strict-Transport-Security "max-age=31536000";
    error_page 497  https://$host$request_uri;
    #SSL-END

    #ERROR-PAGE-START  错误页配置，可以注释、删除或修改
    error_page 404 /404.html;
    #ERROR-PAGE-END

    #REWRITE-START URL重写规则引用,修改后将导致面板设置的伪静态规则失效
    include /www/server/panel/vhost/rewrite/chat.shawntv.co.conf;
    #REWRITE-END

    # ========== 前端路由（Vue Router）==========
    location / {
        try_files $uri $uri/ /index.html;
    }

    # ========== 后端 API 代理 ==========
    location /api {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    }

    # ========== WebSocket 代理（聊天必须）==========
    location /socket.io {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection "upgrade";
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_connect_timeout 60s;
        proxy_send_timeout 60s;
        proxy_read_timeout 60s;
    }

    #禁止访问的文件或目录
    location ~ ^/(\.user.ini|\.htaccess|\.git|\.env|\.svn|\.project|LICENSE|README.md)
    {
        return 404;
    }

    #一键申请SSL证书验证目录相关设置
    location ~ \.well-known{
        allow all;
    }

    #禁止在证书验证目录放入敏感文件
    if ( $uri ~ "^/\.well-known/.*\.(php|jsp|py|js|css|lua|ts|go|zip|tar\.gz|rar|7z|sql|bak)$" ) {
        return 403;
    }

    location ~ .*\.(gif|jpg|jpeg|png|bmp|swf)$
    {
        expires      30d;
        error_log /dev/null;
        access_log /dev/null;
    }

    location ~ .*\.(js|css)?$
    {
        expires      12h;
        error_log /dev/null;
        access_log /dev/null;
    }

    access_log  /www/wwwlogs/chat.shawntv.co.log;
    error_log  /www/wwwlogs/chat.shawntv.co.error.log;
}
```

### 8.4 保存配置

```
点击「保存」按钮
```

### 8.5 测试并重载 Nginx

在终端执行：
```bash
nginx -t && nginx -s reload
```

看到这个就成功了：
```
nginx: configuration file /www/server/nginx/conf/nginx.conf test is successful
```

---

## 步骤 9️⃣ 域名解析

### 9.1 去域名管理后台

```
阿里云：https://dc.console.aliyun.com
腾讯云：https://console.cloud.tencent.com/cns
Cloudflare：https://dash.cloudflare.com
```

### 9.2 添加解析记录

```
点击 chat.shawntv.co → 点击「添加记录」

填写：
┌─────────────────────────────────────────────────────┐
│  记录类型:   A                                       │
│  主机记录:   chat                                    │
│  记录值:     你的服务器IP（例如 47.98.123.456）       │
│  TTL:        默认                                    │
└─────────────────────────────────────────────────────┘

点击「确定」
```

### 9.3 等待生效

```
域名解析需要 1-10 分钟生效

验证方法（在终端执行）：
ping chat.shawntv.co

如果显示你的服务器IP，说明解析成功！
```

---

## 步骤 🔟 申请 SSL 证书（HTTPS）

### 10.1 打开 SSL 设置

```
📍 位置：网站 → 你的网站 → 设置 → 左侧点「SSL」
```

### 10.2 申请免费证书

```
1. 点击「Let's Encrypt」选项卡
2. 勾选你的域名 chat.shawntv.co
3. 点击「申请」
4. 等待 30 秒左右
5. 申请成功后，开启「强制HTTPS」开关
```

### 10.3 如果申请失败

```
原因1: 域名解析还没生效
→ 等 10 分钟再试

原因2: 80端口没开放
→ 往下看「开放防火墙端口」

原因3: 域名没备案（国内服务器）
→ 去域名服务商备案，或换国外服务器
```

---

## 步骤 1️⃣1️⃣ 开放防火墙端口

### 11.1 宝塔面板防火墙

```
📍 位置：左侧「安全」

添加以下端口：
- 80（HTTP）
- 443（HTTPS）
- 3000（后端API，可选）

操作：输入端口号 → 点「放行」
```

### 11.2 云服务器安全组

```
📍 位置：阿里云/腾讯云控制台 → 云服务器 → 安全组

添加入站规则：
- 端口 80，来源 0.0.0.0/0
- 端口 443，来源 0.0.0.0/0
```

---

# 第三部分：完成！

## 🎉 验证部署成功

### 打开浏览器访问

```
https://chat.shawntv.co
```

### 检查清单

| 检查项 | 正常情况 | 异常处理 |
|-------|---------|---------|
| 页面能打开 | 看到登录页面 | 检查 Nginx 配置 |
| 地址栏有🔒 | 显示小锁图标 | 检查 SSL 证书 |
| 能注册账号 | 注册成功跳转 | 检查后端 `pm2 list` |
| 能登录 | 进入聊天界面 | 检查后端日志 |
| 能发消息 | 消息实时显示 | 检查 WebSocket 代理 |

---

# 常见问题速查

## ❌ 页面打不开 / 502 错误

```bash
# 检查后端是否运行
pm2 list

# 如果 status 不是 online，重启
pm2 restart yuwenchat

# 看错误日志
pm2 logs yuwenchat --err
```

## ❌ 页面显示 404

```bash
# 检查 dist 目录是否存在
ls /www/wwwroot/chat.shawntv.co/client/dist/

# 如果不存在，重新构建
cd /www/wwwroot/chat.shawntv.co/client
npm run build
```

## ❌ 注册/登录失败

```bash
# 测试后端是否正常
curl http://127.0.0.1:3000/api/health

# 应该返回 {"status":"ok"}
# 如果不行，检查后端日志
pm2 logs yuwenchat
```

## ❌ 消息发不出去

检查 Nginx 配置里的 `/socket.io` 部分是否正确配置

## ❌ npm install 报错

```bash
# 使用国内镜像
npm config set registry https://registry.npmmirror.com

# 如果是编译错误
apt update && apt install -y build-essential python3

# 重新安装
npm install
```

---

# 一键命令速查表

```bash
# ===== 重新部署（如果搞砸了）=====
cd /www/wwwroot/chat.shawntv.co/server && npm install && \
cd ../client && npm install && npm run build && \
cd ../server && pm2 restart yuwenchat

# ===== 查看后端状态 =====
pm2 list

# ===== 查看后端日志 =====
pm2 logs yuwenchat

# ===== 重启后端 =====
pm2 restart yuwenchat

# ===== 测试后端 =====
curl http://127.0.0.1:3000/api/health

# ===== 测试 Nginx 配置 =====
nginx -t

# ===== 重载 Nginx =====
nginx -s reload
```

---

# 🔄 手动更新到最新版本

> 当有新功能发布时，按照这个指南更新

## 更新前必读

```
⚠️ 更新会保留你的数据！
✅ 聊天记录 - 不会丢失
✅ 用户账号 - 不会丢失
✅ 好友关系 - 不会丢失
✅ 头像设置 - 不会丢失

新版本的数据库变更会自动迁移，不需要手动操作！
```

## 更新步骤（3分钟搞定）

### 方法一：一键更新命令 ⭐推荐⭐

打开宝塔终端，复制粘贴这整段命令：

```bash
# ===== 一键更新命令（全部复制，粘贴，回车）=====

cd /www/wwwroot/chat.shawntv.co && \
echo ">>> 1/5 备份数据库" && \
cp server/data/chat.db server/data/chat.db.backup.$(date +%Y%m%d_%H%M%S) && \
echo ">>> 2/5 拉取最新代码" && \
git pull origin main && \
echo ">>> 3/5 更新后端依赖" && \
cd server && npm install --registry=https://registry.npmmirror.com && \
echo ">>> 4/5 更新前端并重新构建" && \
cd ../client && npm install --registry=https://registry.npmmirror.com && npm run build && \
echo ">>> 5/5 重启后端服务" && \
cd ../server && pm2 restart yuwenchat && \
echo "" && \
echo "========================================" && \
echo "✅ 更新完成！刷新网页即可看到新版本" && \
echo "========================================"
```

### 方法二：分步操作（如果你想了解每一步）

#### 第1步：备份数据库（保险起见）

```bash
cd /www/wwwroot/chat.shawntv.co
cp server/data/chat.db server/data/chat.db.backup
```

#### 第2步：拉取最新代码

```bash
cd /www/wwwroot/chat.shawntv.co
git pull origin main
```

如果提示冲突，执行：
```bash
git stash
git pull origin main
```

#### 第3步：更新后端依赖

```bash
cd /www/wwwroot/chat.shawntv.co/server
npm install
```

#### 第4步：重新构建前端

```bash
cd /www/wwwroot/chat.shawntv.co/client
npm install
npm run build
```

#### 第5步：重启后端

```bash
pm2 restart yuwenchat
```

#### 第6步：验证更新

```bash
# 检查后端状态
pm2 list

# 应该显示 status: online
```

然后刷新网页，检查新功能是否生效！

---

## 更新后常见问题

### ❌ git pull 报错 "local changes would be overwritten"

```bash
# 暂存本地修改
git stash

# 重新拉取
git pull origin main

# 如果需要恢复本地修改
git stash pop
```

### ❌ npm install 报错

```bash
# 清除缓存重试
rm -rf node_modules package-lock.json
npm install --registry=https://registry.npmmirror.com
```

### ❌ 页面没变化

```bash
# 1. 确认前端重新构建了
ls -la /www/wwwroot/chat.shawntv.co/client/dist/

# 2. 清除浏览器缓存（Ctrl+Shift+R 或 Cmd+Shift+R）

# 3. 重载 Nginx
nginx -s reload
```

### ❌ 后端启动失败

```bash
# 查看错误日志
pm2 logs yuwenchat --err --lines 50

# 常见原因：依赖没装好，重新安装
cd /www/wwwroot/chat.shawntv.co/server
rm -rf node_modules
npm install
pm2 restart yuwenchat
```

---

## 回滚到旧版本（如果新版本有问题）

```bash
# 恢复数据库备份
cd /www/wwwroot/chat.shawntv.co/server/data
cp chat.db.backup chat.db

# 回滚代码到上一个版本
cd /www/wwwroot/chat.shawntv.co
git log --oneline -5  # 查看最近5个版本
git checkout <之前的版本号>

# 重新构建
cd client && npm run build
cd ../server && pm2 restart yuwenchat
```

---

# 🔧 完全重装指南（从零开始）

> 如果你的系统出了问题，或者想完全重新部署，按照这个指南操作

## 什么时候需要完全重装？

```
🔸 系统配置搞乱了，想从头来
🔸 换了新服务器
🔸 数据不需要了，想清空重来
🔸 遇到无法解决的错误
```

## 完全重装步骤

### 📋 第一阶段：清理旧环境（如果有的话）

如果服务器上已经部署过，先清理：

```bash
# ===== 1. 停止并删除旧的后端服务 =====
pm2 stop yuwenchat 2>/dev/null
pm2 delete yuwenchat 2>/dev/null
pm2 save

# ===== 2. 备份数据库（如果需要保留数据）=====
# 如果你想保留聊天记录和用户数据：
cp /www/wwwroot/chat.shawntv.co/server/data/chat.db ~/chat.db.backup

# ===== 3. 删除旧代码 =====
rm -rf /www/wwwroot/chat.shawntv.co

# ===== 4. 在宝塔面板删除旧网站 =====
# 手动操作：宝塔 → 网站 → 找到网站 → 删除（不要勾选删除目录）
```

### 📋 第二阶段：全新安装

#### 步骤 1：确认基础软件已安装

在宝塔软件商店确认这些已安装：
- ✅ PM2 管理器
- ✅ Nginx

如果没装，参考上面「步骤 4️⃣」安装。

#### 步骤 2：创建项目目录并下载代码

```bash
# 创建目录
mkdir -p /www/wwwroot/chat.shawntv.co

# 进入目录
cd /www/wwwroot/chat.shawntv.co

# 下载代码（选择一种方式）
```

**方式A：从 GitHub 克隆（推荐）**
```bash
git clone https://github.com/yuwenchat/2026-Rev.git .
```

**方式B：上传压缩包**
```
通过宝塔文件管理器上传 zip 文件，然后解压到当前目录
```

#### 步骤 3：一键部署命令

```bash
# ===== 复制这整段命令执行 =====

cd /www/wwwroot/chat.shawntv.co && \
echo ">>> 1/6 创建数据目录" && \
mkdir -p server/data && \
echo ">>> 2/6 安装后端依赖" && \
cd server && npm install --registry=https://registry.npmmirror.com && \
echo ">>> 3/6 创建环境变量" && \
cat > .env << 'EOF'
PORT=3000
NODE_ENV=production
JWT_SECRET=yuwenchat-secret-key-$(date +%s)-change-me
CLIENT_URL=https://chat.shawntv.co
DB_PATH=./data/chat.db
EOF
echo ">>> 4/6 安装前端依赖" && \
cd ../client && npm install --registry=https://registry.npmmirror.com && \
echo ">>> 5/6 构建前端" && \
npm run build && \
echo ">>> 6/6 启动后端" && \
cd ../server && pm2 start src/index.js --name yuwenchat && pm2 save && \
echo "" && \
echo "========================================" && \
echo "✅ 部署完成！" && \
echo "========================================" && \
echo "下一步：配置网站和 Nginx"
```

**域名已设置为 chat.shawntv.co，如需修改请编辑 server/.env 文件**

#### 步骤 4：恢复数据（如果之前备份了）

```bash
# 如果你之前备份了数据库，想恢复数据：
cp ~/chat.db.backup /www/wwwroot/chat.shawntv.co/server/data/chat.db

# 重启后端让它读取数据
pm2 restart yuwenchat
```

#### 步骤 5：配置网站和 Nginx

按照上面的「步骤 7️⃣」和「步骤 8️⃣」操作：

1. **添加网站**
   - 域名：chat.shawntv.co
   - 根目录：`/www/wwwroot/chat.shawntv.co/client/dist`
   - PHP版本：纯静态

2. **配置 Nginx**
   - 修改配置文件，添加 API 反向代理和 WebSocket 支持
   - 参考「步骤 8️⃣」的配置

3. **申请 SSL 证书**
   - 宝塔 → 网站 → 设置 → SSL → Let's Encrypt → 申请

4. **测试访问**
   - 浏览器打开 https://chat.shawntv.co
   - 应该看到登录页面

---

## 📋 快速检查清单

部署完成后，逐项检查：

```
□ pm2 list 显示 yuwenchat 状态为 online
□ https://域名 能打开登录页面
□ 能成功注册新用户
□ 能成功登录
□ 能添加好友
□ 能发送消息
□ 对方能收到消息（实时）
□ 刷新页面后聊天记录还在
```

如果某项失败，查看日志：
```bash
# 后端日志
pm2 logs yuwenchat --lines 100

# Nginx 错误日志
tail -100 /www/wwwlogs/chat.shawntv.co.error.log
```

---

# 👨‍💼 管理员后台

> 管理用户、群组、查看统计数据

## 默认管理员账号

```
⚠️⚠️⚠️ 首次部署后必须立即修改密码！⚠️⚠️⚠️

用户名: admin
密码:   admin1

登录后第一件事：设置 → 修改密码！
```

## 访问管理后台

**⚠️ 必须先登录管理员账号才能访问！直接访问 /admin 会跳转到登录页。**

### 步骤：

1. 打开 https://chat.shawntv.co
2. 用管理员账号登录（用户名: `admin`，密码: `admin1`）
3. 登录成功后，在聊天页面**左上角**会看到一个 ⚙️ 齿轮图标
4. 点击齿轮图标进入管理后台

```
┌─────────────────────────────────────┐
│  🏠 [⚙️]  ← 点这个齿轮进入管理后台   │
│  ────────                           │
│  我的代码: ABC123                    │
│  ────────                           │
│  [添加好友] [新建群组] [加入群组]     │
│                                     │
│  好友                                │
│  ├── alice                          │
│  └── bob                            │
│                                     │
└─────────────────────────────────────┘
```

**注意：** 普通用户登录后看不到齿轮图标，只有管理员才能看到。

## 管理后台功能

### 📊 数据统计
- 用户总数
- 群组总数
- 消息总数
- 好友关系数
- 最近7天新增用户
- 最近7天消息数

### 👥 用户管理
- 查看所有用户列表
- 查看用户详情（好友数、群组数、消息数）
- 修改用户名
- 设置/取消管理员权限
- 删除用户

**⚠️ 注意：** 不能通过管理后台重置用户密码！因为用户的私钥是用密码加密的，重置密码会导致用户无法解密之前的消息。用户只能自己在设置里修改密码。

### 👥 群组管理
- 查看所有群组
- 查看群组成员数和消息数
- 删除群组

### 💬 消息管理
- 查看所有消息（分页）
- 删除消息

**⚠️ 注意：** 消息内容是加密的，管理员看到的是加密后的乱码，无法查看明文！

## 添加其他管理员

1. 让用户先注册普通账号
2. 用现有管理员登录管理后台
3. 在用户列表找到该用户
4. 点击编辑，勾选"管理员"
5. 保存

## 管理后台截图示意

```
┌─────────────────────────────────────────────────────────────┐
│  YuwenChat 管理后台                          [返回聊天]      │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐    │
│  │  用户数   │  │  群组数   │  │  消息数   │  │  好友数   │    │
│  │    25    │  │    8     │  │   1,234  │  │    42    │    │
│  └──────────┘  └──────────┘  └──────────┘  └──────────┘    │
│                                                             │
│  [用户管理]  [群组管理]  [消息记录]  [系统更新]              │
│                                                             │
│  用户列表                                                    │
│  ┌─────────────────────────────────────────────────────┐   │
│  │ 用户名      好友代码     注册时间        操作       │   │
│  ├─────────────────────────────────────────────────────┤   │
│  │ admin      ABC123     2024-01-01    [编辑] [删除]   │   │
│  │ alice      XYZ789     2024-01-02    [编辑] [删除]   │   │
│  │ bob        DEF456     2024-01-03    [编辑] [删除]   │   │
│  └─────────────────────────────────────────────────────┘   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

# 🎊 恭喜！

如果你看到了聊天界面，说明部署成功了！

记得：
1. ⚠️ **立即修改 admin 默认密码！**
2. 定期备份数据库（`/www/wwwroot/chat.shawntv.co/server/data/chat.db`）
3. 有问题看日志：`pm2 logs yuwenchat`

享受你的私密聊天应用吧！

---

# 📁 项目文件结构

> 了解每个文件夹的作用，方便备份和排查问题

## 完整目录结构

```
/www/wwwroot/chat.shawntv.co/          # 项目根目录
│
├── client/                             # 🖥️ 前端代码
│   ├── src/                            # 前端源代码
│   │   ├── components/                 # Vue 组件
│   │   ├── views/                      # 页面视图
│   │   ├── stores/                     # Pinia 状态管理
│   │   └── utils/                      # 工具函数（加密、API等）
│   ├── dist/                           # ⭐ 构建后的静态文件（Nginx 指向这里）
│   │   ├── index.html
│   │   └── assets/
│   └── package.json
│
├── server/                             # 🔧 后端代码
│   ├── src/                            # 后端源代码
│   │   ├── index.js                    # 入口文件
│   │   ├── db/                         # 数据库相关
│   │   └── routes/                     # API 路由
│   │
│   ├── data/                           # 📦 数据存储目录
│   │   └── chat.db                     # ⭐ SQLite 数据库（重要！）
│   │
│   ├── uploads/                        # 📎 用户上传文件
│   │   ├── avatars/                    # 用户头像
│   │   │   └── *.jpg, *.png
│   │   └── *.jpg, *.png, *.pdf...      # 聊天中发送的文件
│   │
│   ├── .env                            # 环境变量配置
│   └── package.json
│
├── DEPLOY_GUIDE.md                     # 本部署文档
├── README.md                           # 项目说明
└── PLAN.md                             # 开发计划
```

## 关键文件说明

### 💾 数据库文件（最重要！）

```
server/data/chat.db
```

这是 SQLite 数据库文件，包含：
- 所有用户账号和加密后的密码
- 所有加密的聊天消息
- 好友关系和好友请求
- 群组信息和成员关系
- 用户设置（头像颜色等）

**⚠️ 这个文件丢失 = 所有数据丢失！务必定期备份！**

### 📎 上传文件目录

```
server/uploads/           # 聊天附件（图片、文件等）
server/uploads/avatars/   # 用户头像
```

用户发送的图片和文件都存在这里。

### 🖥️ 前端构建文件

```
client/dist/
```

这是 `npm run build` 生成的静态文件，Nginx 直接读取这个目录。

### ⚙️ 环境变量

```
server/.env
```

包含：
- `PORT` - 后端端口（默认3000）
- `JWT_SECRET` - JWT 密钥
- `CLIENT_URL` - 前端地址
- `DB_PATH` - 数据库路径

---

## 🔒 需要备份的文件

只需要备份这两个目录，就能完整恢复所有用户数据：

```bash
# 方法1：单独备份
cp server/data/chat.db ~/backup/
cp -r server/uploads/ ~/backup/

# 方法2：打包备份（推荐）
tar -czvf ~/yuwenchat-backup-$(date +%Y%m%d).tar.gz \
    server/data/ \
    server/uploads/
```

### 备份内容清单

| 文件/目录 | 内容 | 是否必须备份 |
|----------|------|-------------|
| `server/data/chat.db` | 用户、消息、好友、群组 | ✅ 必须 |
| `server/uploads/` | 用户上传的图片和文件 | ✅ 必须 |
| `server/.env` | 环境变量配置 | ⚡ 建议 |
| `client/dist/` | 前端构建文件 | ❌ 可重新构建 |
| 其他代码文件 | 源代码 | ❌ 可从Git拉取 |

---

## 🔄 恢复数据

如果需要在新服务器恢复数据：

```bash
# 1. 先完成基础部署（参考上面的安装步骤）

# 2. 停止后端
pm2 stop yuwenchat

# 3. 恢复数据库
cp ~/backup/chat.db /www/wwwroot/chat.shawntv.co/server/data/

# 4. 恢复上传文件
cp -r ~/backup/uploads/* /www/wwwroot/chat.shawntv.co/server/uploads/

# 5. 重启后端
pm2 restart yuwenchat
```

---

## 📊 数据库表结构（参考）

`chat.db` 包含以下数据表：

| 表名 | 内容 |
|-----|------|
| `users` | 用户账号、加密密码、公钥、好友代码、头像 |
| `messages` | 加密的聊天消息 |
| `friendships` | 好友关系和好友请求 |
| `groups` | 群组信息 |
| `group_members` | 群组成员 |
| `group_messages` | 群组消息 |

**注意：** 消息内容是端对端加密的，即使直接查看数据库也无法读取明文。

